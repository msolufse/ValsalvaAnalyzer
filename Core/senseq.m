function [sens, rout, sol] = senseq(pars,data)
% function senseq.m
% Input: model parameters and data structure
% Output: sensitivity matrix, model residual, and solution
% Uses: model_sol to solve the ODEs and predict heart rate.
% Description: Computes the sensitivity matrix dy/dpars.
% For relative sensitivity matrix dy/dlog(pars) = dy/dpars*pars, set 
% pars = log(pars) in DriverBasic_sens.

[HR,rout] = model_sol(pars,data);

% Depending on form of y, the sensitivity matrix could be dymodel/dpars or 
% dr/dpars where r = ymodel - ydata
[sens] = diffjac(pars,@myfun,rout,data);


function rout = myfun(pars,data)

[~,rout] = model_sol(pars,data);
